...
def check_cache_and_permissions():
    ws_url = get_target()
    client = CDPClient(ws_url)

    cached_perms = client.eval_js("return localStorage.getItem('tab_permissions');")
    print(f"Cached Tab Permissions in localStorage: {cached_perms}")

    # Let's clear localStorage to let fresh TAB_PERMISSIONS take effect!
    client.eval_js("localStorage.removeItem('tab_permissions'); window.location.reload();")
    time.sleep(1.0)

    print("Reloaded page. Testing profile tab again...")
    client.eval_js("window.location.hash = '#/profil'; window.dispatchEvent(new Event('hashchange'));")
    time.sleep(0.5)
    ...
    """)
    print(f"Profile Page Rendered Check: {json.dumps(res, indent=2, ensure_ascii=False)}")

if __name__ == '__main__':
    check_cache_and_permissions()
