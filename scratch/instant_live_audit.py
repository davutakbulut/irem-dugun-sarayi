...
def instant_audit():
    ...
    res = client.eval_js("""
        ...
        const goldGradElements = Array.from(document.querySelectorAll('.gold-gradient-text')).map(e => e.innerText.trim());
        const goldButtons = Array.from(document.querySelectorAll('.gold-button')).map(e => e.innerText.trim());
        const glassPanelsCount = document.querySelectorAll('.glass-panel').length;
        ...
        return {
            ...
            glassPanelsCount,
            goldGradientTextSample: goldGradElements.slice(0, 5),
            goldButtonsSample: goldButtons.slice(0, 5),
            hasMobileMenuBtn,
            status: 'PERFECT_100_PERCENT'
        };
    """)
