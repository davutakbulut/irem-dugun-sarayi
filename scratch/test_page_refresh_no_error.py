...
def test_refresh_no_errors():
    ...
    res = client.eval_js("""
        ...
        const isErrorState = bodyText.includes('ReferenceError') || bodyText.includes('Uncaught');

        return {
            appRenderedSuccessfully: hasChildren && !isErrorState,
            rootChildCount: root ? root.children.length : 0,
            hasReferenceErrorText: isErrorState
        };
    """)
    print("Refresh Test Result:", json.dumps(res, indent=2, ensure_ascii=False))
