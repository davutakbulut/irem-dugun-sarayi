...
def test_profile_page():
    ...
    print("--- 1. Testing hash route navigation to #/profil ---")
    client.eval_js("window.location.hash = '#/profil'; return window.location.href;")
    time.sleep(0.5)

    current_hash = client.eval_js("return window.location.hash;")
    print(f"Current hash: {current_hash}")

    page_text = client.eval_js("return document.body.innerText;")
    print("Page preview (first 400 chars):")
    print(page_text[:400] if page_text else "EMPTY")

    print("\n--- 2. Alternatively testing profile dropdown click ---")
    dropdown_res = client.eval_js("""
        const profileBtn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('Profil'));
        if (profileBtn) {
            profileBtn.click();
            setTimeout(() => {
                const editOpt = Array.from(document.querySelectorAll('button, a, div')).find(el => el.innerText && el.innerText.includes('Profilimi Düzenle'));
                if (editOpt) editOpt.click();
            }, 200);
            return { profileBtnFound: true };
        }
        return { profileBtnFound: false };
    """)
    time.sleep(0.5)
    print(f"Dropdown click result: {dropdown_res}")

    print("\n--- 3. Checking ProfileComponent Form Inputs ---")
    inputs_info = client.eval_js("""
        ...
        return { inputs, headings, buttons };
    """)
    print(f"Form & Page Elements: {json.dumps(inputs_info, indent=2, ensure_ascii=False)}")
