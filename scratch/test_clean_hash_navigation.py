...
def test_clean_hash_navigation():
    ws_url = get_target()
    client = CDPClient(ws_url)

    # 1. Navigate to reservation form with query params
    client.call("Page.navigate", {"url": "http://localhost:8000/#/rezervasyon-olustur?editId=RES-2026-001"})
    time.sleep(1.0)
    hash1 = client.eval_js("return window.location.hash;")

    # 2. Click Dashboard link in sidebar / logo
    client.eval_js("""
        const logo = document.querySelector('div[aria-label="İrem Düğün Sarayı Ana Sayfa"]');
        if (logo) logo.click();
    """)
    time.sleep(0.8)
    hash2 = client.eval_js("return window.location.hash;")

    # 3. Navigate to venues via sidebar link
    client.eval_js("""
        const venueBtn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('Salonlarım') || b.innerText.includes('Düğün Salonları'));
        if (venueBtn) venueBtn.click();
    """)
    time.sleep(0.8)
    hash3 = client.eval_js("return window.location.hash;")

    print("Initial Hash with editId:", hash1)
    print("Hash after clicking Dashboard:", hash2)
    print("Hash after clicking Venues:", hash3)

if __name__ == '__main__':
    test_clean_hash_navigation()
