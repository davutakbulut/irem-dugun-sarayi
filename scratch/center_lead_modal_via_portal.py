...
if start_idx != -1 and end_idx != -1:
    new_lead_modal_code = """    function LeadModal({ isOpen, onClose, defaultEventType = 'Düğün', onSaveQuoteRequest, showToast }) {
      ...
      // Lock body scrolling when modal is open to ensure device viewport centering
      useEffect(() => {
        if (isOpen) {
...
✨ Web sitemiz üzerinden "Ücretsiz Teklif Al" formu ile gönderilmiştir.`;
      ...
      const modalJSX = (
        <div 
          className="fixed inset-0 top-0 left-0 w-screen h-screen z-[999999] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 overflow-y-auto animate-fade-in"
          onClick={(e) => {
            if (e.target === e.currentTarget) onClose();
          }}
        >
          <div className="bg-[#0f172a] border border-[#c5a059]/40 rounded-3xl max-w-xl w-full p-6 sm:p-8 space-y-6 text-white shadow-2xl relative my-auto max-h-[90vh] overflow-y-auto custom-scrollbar">
            <button 
              onClick={onClose} 
              className="absolute top-5 right-5 w-8 h-8 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white flex items-center justify-center text-sm font-bold transition cursor-pointer z-10"
              title="Kapat"
            >
              ✕
            </button>
...
"""
    ...
    print("Successfully converted LeadModal to Portal with viewport scroll lock!")
