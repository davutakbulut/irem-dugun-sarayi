...
# Replace Lightbox Modal rendering in MediaComponent with ReactDOM.createPortal and fixed viewport positioning
old_lightbox = """      {/* LIGHTBOX MODAL VIEW */}
...
new_lightbox = """      {/* FULLSCREEN VIEWPORT PORTAL LIGHTBOX MODAL */}
      {lightboxIndex !== null && mediaList[lightboxIndex] && typeof ReactDOM !== 'undefined' && ReactDOM.createPortal(
        <div className="fixed inset-0 w-screen h-screen z-[999999] bg-slate-950/98 backdrop-blur-xl flex items-center justify-center p-4 sm:p-8 animate-fade-in pointer-events-auto">
          ...
          <button
            type="button"
            onClick={() => setLightboxIndex(null)}
            className="fixed top-4 right-4 sm:top-6 sm:right-6 w-12 h-12 rounded-full bg-red-600/90 hover:bg-red-500 hover:scale-110 text-white font-extrabold text-2xl shadow-2xl flex items-center justify-center transition cursor-pointer z-[1000000] border-2 border-white/30"
            title="Kapat (ESC)"
            aria-label="Kapat"
          >
            ✕
...
if old_lightbox in html:
    ...
    print("Replaced Lightbox Modal rendering with Fullscreen Viewport Portal!")

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Updated index.html fullscreen portal modal successfully!")
