...
# 1. Update TAB_TO_SLUG, SLUG_TO_TAB, TAB_LABELS, TAB_PERMISSIONS for 'roles'
if "'roles': 'roller'" not in html:
    html = html.replace(
    ...
    print("Added route mappings for 'roles'")

# 2. Add 'roles' to Sidebar menu under YÖNETİM & AYARLAR
old_sidebar_settings = "                        { id: 'users', label: 'Kullanıcı Yönetimi', icon: 'shield', fallbackEmoji: '🛡️' },"
new_sidebar_settings = "                        { id: 'roles', label: 'Rol Yönetimi & İzinler', icon: 'shield', fallbackEmoji: '🛡️', badge: 'YENİ' },\n                        { id: 'users', label: 'Kullanıcı Yönetimi', icon: 'user', fallbackEmoji: '👥' },"

if old_sidebar_settings in html:
    html = html.replace(old_sidebar_settings, new_sidebar_settings)
    print("Added 'roles' to Sidebar menu")

# 3. Add 'roles' to Topbar Mega Menu (HorizontalNavbarComponent)
old_topbar_settings = "{ id: 'users', label: 'Personel (RBAC Yetkileri)', desc: 'Kullanıcı rolleri, şifre tanımları ve erişim matrisi', icon: 'shield', fallbackEmoji: '🛡️' },"
new_topbar_settings = "{ id: 'roles', label: 'Rol Yönetimi (Sistem Rolleri)', desc: 'Rol ekleme, düzenleme, silme ve yetki kısıtlama paneli', icon: 'shield', fallbackEmoji: '🛡️', badge: 'ADMIN' },\n            { id: 'users', label: 'Personel (RBAC Yetkileri)', desc: 'Kullanıcı tanımları ve şifre yönetimi', icon: 'user', fallbackEmoji: '👥' },"

if old_topbar_settings in html:
    html = html.replace(old_topbar_settings, new_topbar_settings)
    print("Added 'roles' to Topbar menu")


# 4. Add RolesPageComponent definition before SettingsComponent
roles_page_component_code = """
    // --- DEDICATED ROLES PAGE COMPONENT (SADECE ADMİN ROL YÖNETİM SAYFASI) ---
    function RolesPageComponent({ activeRole, roles, users = [], tabPermissions, onAddRole, onEditRole, onDeleteRole, onToggleTabPermission, showToast, navigateTo }) {
        ...
        <div className="w-full space-y-6 animate-fade-in pb-16">
          ...
          <div className="glass-panel p-6 rounded-3xl space-y-4 border border-amber-500/20 shadow-sm">
            <h3 className="font-heading font-extrabold text-base text-slate-800 dark:text-gray-100 flex items-center space-x-2">
              ...
              <span>➕ Yeni Sistem Rolü Tanımla</span>
            </h3>
              ...
              <div className="bg-white dark:bg-brand-card border border-amber-500/40 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl">
                <div className="flex justify-between items-center border-b pb-3 border-slate-200 dark:border-brand-border">
                  <h3 className="font-bold text-sm text-slate-800 dark:text-gray-100 flex items-center space-x-2">
                    <span>✏️ Rol Unvanını Güncelle ({editingRole.id})</span>
                  </h3>
...
# Insert RolesPageComponent before SettingsComponent in index.html
if "function RolesPageComponent" not in html:
    html = html.replace("function SettingsComponent", roles_page_component_code + "\n\n    function SettingsComponent")
    print("Inserted RolesPageComponent into index.html")

# 5. Render RolesPageComponent when activeTab === 'roles'
render_roles_page = """                  {activeTab === 'roles' && (
                    <RolesPageComponent
...
if "{activeTab === 'roles' && (" not in html:
    html = html.replace(
        "{activeTab === 'users' && (",
        render_roles_page + "\n\n                  {activeTab === 'users' && ("
    )
    print("Added RolesPageComponent renderer to main layout")

# 6. Fix ReferenceError: onDeleteRole is not defined in SettingsComponent
# In SettingsComponent, remove direct onDeleteRole call or ensure onEditRole and onDeleteRole are in props
html = html.replace(
    "function SettingsComponent({ activeRole, roles, tabPermissions, onAddRole, onToggleTabPermission",
    "function SettingsComponent({ activeRole, roles, tabPermissions, onAddRole, onEditRole, onDeleteRole, onToggleTabPermission"
)

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Updated index.html with dedicated RolesPageComponent successfully!")
