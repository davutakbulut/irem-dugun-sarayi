...
def verify_emerald_contrast():
    ...
    # Set Emerald Royal theme
    client.eval_js("document.documentElement.setAttribute('data-ui-theme', 'emerald-royal');")
    time.sleep(0.3)

    res = client.eval_js("""
        const navLink = document.querySelector('nav a') || document.querySelector('a');
        const logo = document.querySelector('.gold-gradient-text');
        const navColor = navLink ? getComputedStyle(navLink).color : null;
        const logoBg = logo ? getComputedStyle(logo).backgroundImage : null;

        return {
            themeAttribute: document.documentElement.getAttribute('data-ui-theme'),
            navLinkTextColor: navColor,
            headerLogoGradient: logoBg,
            navLinkIsMintGreen: navColor === 'rgb(167, 243, 208)', // #A7F3D0
            logoGradientActive: logoBg && logoBg.includes('gradient'),
            zeroColorLeakSuccess: navColor === 'rgb(167, 243, 208)' && logoBg && logoBg.includes('gradient')
        };
    """)
