...
pattern = re.compile(
    r"// CRUD HANDLERS\s+const handleSaveVenue = \(vObj\) => \{[\s\S]*?const handleDeleteService = \(sIdOrObj\) => \{[\s\S]*?showToast\('Ek Hizmet Başarıyla Silindi\.'\);\s*\}\s*\}\);\s*\}",
    re.MULTILINE
)

new_crud = """// CRUD HANDLERS (DIRECT MARIADB SYNC)
...
for h_file in html_files:
    ...
    if pattern.search(content):
        content = pattern.sub(new_crud, content)
        with open(h_file, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Replaced CRUD handlers in {h_file}")
    else:
