...
def test_sidebar_top_zero():
    ...
    routes = ['#/dashboard', '#/salonlar', '#/ek-hizmetler', '#/musteriler', '#/ayarlar', '#/simulasyon-500']
    results = {}

    for r in routes:
        client.eval_js(f"window.location.hash = '{r}'; window.dispatchEvent(new Event('hashchange'));")
        time.sleep(0.3)
        res = client.eval_js("""
