...
def test_roles_and_views():
    ws_url = get_target()
    client = CDPClient(ws_url)
    
    results = {}
    
    # Test 1: Check Role Switches
    roles = ['Admin 👑', 'Satışçı 💼', 'Sosyal Medya 📸', 'Müşteri <ctrl42>']
    role_results = {}
    for r in ['Admin 👑', 'Satışçı 💼', 'Sosyal Medya 📸', 'Müşteri 💑']:
        clicked = client.eval_js(f"""
            const btns = Array.from(document.querySelectorAll('button'));
            const target = btns.find(b => b.innerText.includes("{r.split(' ')[0]}"));
            if (target) {{ target.click(); true; }} else {{ false; }}
        """)
        time.sleep(0.3)
        body_text = client.eval_js("document.body.innerText.substring(0, 300)")
        role_results[r] = {"clicked": clicked, "preview": body_text[:100].replace('\n', ' ')}
    ...
    # Test 2: Check Navigation items clicking
    tabs_to_test = ['🏛️Düğün Salonlarım', '📝➕ Yeni Rezervasyon', '💰Finans & Fatura', '⚡Önbellek & Performans']
    tab_results = {}
    for tab in tabs_to_test:
        clicked = client.eval_js(f"""
            const items = Array.from(document.querySelectorAll('a, button, div'));
            const item = items.find(el => el.innerText && el.innerText.includes("{tab.replace('⚡', '').replace('🏛️', '').replace('📝➕ ', '').replace('💰', '')}"));
            if (item) {{ item.click(); true; }} else {{ false; }}
        """)
        time.sleep(0.3)
        h_text = client.eval_js("document.querySelector('h1, h2, h3') ? document.querySelector('h1, h2, h3').innerText : ''")
        tab_results[tab] = {"clicked": clicked, "header": h_text}
