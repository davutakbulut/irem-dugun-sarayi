...
def test_theme_isolation():
    ...
    # 1. Reset theme to default gold
    client.call("Page.navigate", {"url": "http://localhost:8000/"})
    time.sleep(1.2)
    client.eval_js("document.documentElement.removeAttribute('data-ui-theme');")
    time.sleep(0.3)

    # Check Default Gold Theme
    default_gold_res = client.eval_js("""
        const glassPanel = document.querySelector('.glass-panel');
        const goldBtn = document.querySelector('.gold-button');
        const glassRadius = glassPanel ? getComputedStyle(glassPanel).borderRadius : null;
        const btnRadius = goldBtn ? getComputedStyle(goldBtn).borderRadius : null;

        return {
            mode: 'Default Gold Theme (Klasik Şampanya Altını)',
            glassBorderRadius: glassRadius,
            buttonBorderRadius: btnRadius,
            hasRoundedCurves: glassRadius !== '0px' && btnRadius !== '0px',
            themeAttribute: document.documentElement.getAttribute('data-ui-theme')
        };
    """)
    print("1. Default Gold Theme:", json.dumps(default_gold_res, indent=2, ensure_ascii=False))

    # 2. Switch to Quiet Luxury
    client.eval_js("document.documentElement.setAttribute('data-ui-theme', 'elite-luxury');")
    time.sleep(0.3)

    quiet_luxury_res = client.eval_js("""
        const glassPanel = document.querySelector('.glass-panel');
        const goldBtn = document.querySelector('.gold-button');
        const glassRadius = glassPanel ? getComputedStyle(glassPanel).borderRadius : null;
        const btnRadius = goldBtn ? getComputedStyle(goldBtn).borderRadius : null;
        ...
        return {
            mode: 'Quiet Luxury (Dark Architecture)',
            glassBorderRadius: glassRadius,
            buttonBorderRadius: btnRadius,
            isZeroPxSharp: glassRadius === '0px' && btnRadius === '0px',
            themeAttribute: document.documentElement.getAttribute('data-ui-theme')
        };
    """)
    print("2. Quiet Luxury Theme:", json.dumps(quiet_luxury_res, indent=2, ensure_ascii=False))

    # 3. Switch to Nordic Clarity
    client.eval_js("document.documentElement.setAttribute('data-ui-theme', 'nordic-light');")
    time.sleep(0.3)

    nordic_light_res = client.eval_js("""
        const glassPanel = document.querySelector('.glass-panel');
        const goldBtn = document.querySelector('.gold-button');
        const glassRadius = glassPanel ? getComputedStyle(glassPanel).borderRadius : null;
        const btnRadius = goldBtn ? getComputedStyle(goldBtn).borderRadius : null;
        ...
        return {
            mode: 'Nordic Clarity (Light Architecture)',
            glassBorderRadius: glassRadius,
            buttonBorderRadius: btnRadius,
            isZeroPxSharp: glassRadius === '0px' && btnRadius === '0px',
            themeAttribute: document.documentElement.getAttribute('data-ui-theme')
    ...
    """)
    print("3. Nordic Clarity Theme:", json.dumps(nordic_light_res, indent=2, ensure_ascii=False))
