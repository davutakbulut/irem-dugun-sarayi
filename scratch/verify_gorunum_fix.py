...
def verify_gorunum():
    ...
    print("Clearing local storage & navigating to http://localhost:8000/#/ayarlar/gorunum ...")
    ...
    res = client.eval_js("""
        ...
        const isWhiteScreen = domCount < 20 || bodyText.length < 50;

        const paletteButtons = Array.from(document.querySelectorAll('button')).filter(b => b.innerText.includes('#') || b.innerText.includes('rounded-') || b.innerText.includes('👑') || b.innerText.includes('💎') || b.innerText.includes('⬛') || b.innerText.includes('🪙') || b.innerText.includes('⚡') || b.innerText.includes('🌲'));
        const paletteNames = paletteButtons.map(b => b.innerText.trim().replace(/\\n/g, ' '));
        const sharpBadges = Array.from(document.querySelectorAll('span')).filter(s => s.innerText.includes('rounded-')).map(s => s.innerText.trim());

        return {
            title,
            isWhiteScreen,
            totalDomNodes: domCount,
            bodyTextSnippet: bodyText.substring(0, 200).replace(/\\n/g, ' '),
            paletteButtonsCount: paletteButtons.length,
            paletteNamesList: paletteNames,
            sharpBadgesCount: sharpBadges.length,
            sharpBadgesList: sharpBadges,
