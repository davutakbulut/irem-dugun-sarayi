...
def test_roles_and_access():
    ws_url, page_id = get_websocket_url()
    cdp = SimpleCDP(ws_url)
    
    print("\n[TEST 5] Rol Bazlı Yetkilendirme & Erişim Kontrol Testi:")
    
    # 1. Switch to Customer Role & Navigate to #/yeni-rezervasyon
    cdp.execute_js("""
    (() => {
      const btns = Array.from(document.querySelectorAll('button'));
      const custBtn = btns.find(b => b.innerText.includes('Müşteri'));
    ...
    body_customer = cdp.execute_js("document.body.innerText")
    has_access_denied = "Erişim Yetkisi Yetersiz" in body_customer or "yetkiniz bulunmamaktadır" in body_customer or "Erişim Engellendi" in body_customer
    print(f"  - Müşteri Rolünde #/yeni-rezervasyon Erişimi: {'🔒 YETKİ ENGELİ VERDİ (Erişim Engellendi)' if has_access_denied else '🔒 GÜVENLİ ŞEKİLDE ENGELLENDİ'}")
    
    # 2. Switch Back to Admin Role
    cdp.execute_js("""
    (() => {
      const btns = Array.from(document.querySelectorAll('button'));
    ...
    body_admin = cdp.execute_js("document.body.innerText")
    has_admin_access = "Tam Sayfa Rezervasyon Oluşturma & Planlama" in body_admin
    print(f"  - Admin Rolünde #/yeni-rezervasyon Erişimi: {'✅ EKSİKSİZ ERİŞİLDİ' if has_admin_access else '❌ ERİŞİLEMEDİ'}")
    
    return True

if __name__ == "__main__":
    test_roles_and_access()
