...
def test_draft_ref_preservation():
    ws_url = get_target()
    client = CDPClient(ws_url)

    client.call("Page.navigate", {"url": "http://localhost:8000/#/rezervasyon-listesi"})
    time.sleep(1.2)

    res = client.eval_js("""
        const draftButtons = Array.from(document.querySelectorAll('button')).filter(b => b.innerText.includes('Devam Et & Tamamla'));
        if (draftButtons.length === 0) return { draftFound: false };

        const targetBtn = draftButtons[0];
        // Get expected refKey from parent card
        const card = targetBtn.closest('.bg-white, .dark\\:bg-brand-card') || targetBtn.parentElement.parentElement;
        const cardText = card ? card.innerText : '';
        const match = cardText.match(/🔑\s*([A-Za-z0-9_-]+)/);
        const expectedRef = match ? match[1] : null;
    ...
    time.sleep(0.8)
    res_form = client.eval_js("""
        const refSpan = document.querySelector('span.tracking-wider');
        return {
            refInUI: refSpan ? refSpan.innerText.trim() : null,
            hashNow: window.location.hash
        };
    """)

    print("Navigation Result:", json.dumps(res, indent=2, ensure_ascii=False))
    print("Form Ref Key Verification:", json.dumps(res_form, indent=2, ensure_ascii=False))

if __name__ == '__main__':
    test_draft_ref_preservation()
