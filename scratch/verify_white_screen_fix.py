...
def verify_rendering_and_colors():
    ws_url = get_target()
    client = CDPClient(ws_url)

    print("Reloading page and waiting for React DOM...")
    ...
    res = client.eval_js("""
        ...
        const venuesCount = document.querySelectorAll('[class*="glass-panel"], .grid > div, img').length;
        const goldButtons = Array.from(document.querySelectorAll('.gold-button')).map(b => b.innerText.trim());
        const hasMobileMenuBtn = Array.from(document.querySelectorAll('button')).some(b => b.innerText.includes('☰'));
        const isWhiteScreen = !hasChildren || domCount < 20 || bodyText.length < 50;

        return {
            title,
            isWhiteScreen: isWhiteScreen,
            reactRootMounted: hasChildren,
            totalDomNodes: domCount,
            renderedElementsCount: venuesCount,
            goldButtonsSample: goldButtons.slice(0, 5),
            hasMobileMenuButton: hasMobileMenuBtn,
            bodyTextPreview: bodyText.substring(0, 200).replace(/\\n/g, ' ')
        };
    """)
