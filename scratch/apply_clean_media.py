...
# 1. Add mock media items to INITIAL_RESERVATIONS
...
# 2. Add SVG icons (qrcode, download, copy, video, play) to NordicSvgMap
...
# 3. Locate MediaComponent and replace cleanly
start_marker = "function MediaComponent({ reservations = [], showToast = () => {} }) {"
...
new_media_component = """function MediaComponent({ reservations = [], setReservations = () => {}, activeRole = 'admin', showToast = () => {} }) {
  ...
  const handleBulkDownload = () => {
    ...
    a.download = 'Etkinlik_Medya_Arsivi.txt';
    a.click();
  };
        ...
        <div className="glass-panel p-4 rounded-2xl border border-slate-200 dark:border-brand-border/40 flex flex-col sm:flex-row justify-between items-center gap-3">
          ...
          <div className="flex items-center space-x-1 bg-slate-100 dark:bg-brand-dark p-1 rounded-xl w-full sm:w-auto justify-end">
            <button
              ...
              className={activeTabSub === 'gallery' ? 'px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center space-x-1 gold-button shadow-xs' : 'px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center space-x-1 text-slate-600 dark:text-gray-400'}
            >
            ...
            <button
              ...
              className={activeTabSub === 'ban_list' ? 'px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center space-x-1 bg-red-500 text-white shadow-xs' : 'px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center space-x-1 text-slate-600 dark:text-gray-400'}
            >
...
# Update MediaComponent invocation props in App component
old_inv = '<MediaComponent reservations={reservations} showToast={showToast} />'
new_inv = '<MediaComponent reservations={reservations} setReservations={setReservations} activeRole={activeRole} showToast={showToast} />'
if old_inv in updated_html:
    updated_html = updated_html.replace(old_inv, new_inv)

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(updated_html)

print("Applied clean media update script successfully!")
