...
def test_nordic_light_tokens():
    ...
    # Set Admin role & set Nordic Light Theme
    client.eval_js("""
    ...
    res = client.eval_js("""
        const htmlAttr = document.documentElement.getAttribute('data-ui-theme');
        const bodyBg = getComputedStyle(document.body).backgroundColor;
        const goldBtn = document.querySelector('.gold-button');
        const goldBtnBg = goldBtn ? getComputedStyle(goldBtn).backgroundColor : null;
        const goldBtnColor = goldBtn ? getComputedStyle(goldBtn).color : null;
        const goldBtnRadius = goldBtn ? getComputedStyle(goldBtn).borderRadius : null;
        const glassPanel = document.querySelector('.glass-panel');
        const glassBg = glassPanel ? getComputedStyle(glassPanel).backgroundColor : null;
        const glassRadius = glassPanel ? getComputedStyle(glassPanel).borderRadius : null;

        return {
            uiThemeAttribute: htmlAttr,
            primaryLightBodyBg: bodyBg,
            secondaryLightCardBg: glassBg,
            primaryAccentButtonBg: goldBtnBg,
            primaryAccentButtonTextColor: goldBtnColor,
            zeroPxRadiusCheck: glassRadius === '0px' && goldBtnRadius === '0px',
            nordicLightActive: htmlAttr === 'nordic-light'
        };
    """)
