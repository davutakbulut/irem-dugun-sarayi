...
def test_edit_id_flow():
    ...
    res = client.eval_js("""
        ...
        if (editBtns.length === 0) return { editFound: false };
        editBtns[0].click();
        return {
            editFound: true,
    ...
    res_form = client.eval_js("""
        const nameInput = document.querySelector('input[placeholder*="Ahmet"]');
        const phoneInput = document.querySelector('input[placeholder*="532"]');
        const heading = document.querySelector('h2');
        return {
            headingText: heading ? heading.innerText : null,
            hashNow: window.location.hash,
            nameVal: nameInput ? nameInput.value : null,
            phoneVal: phoneInput ? phoneInput.value : null
        };
    """)

    print("Navigation Result:", json.dumps(res, indent=2, ensure_ascii=False))
    print("Edit Form State:", json.dumps(res_form, indent=2, ensure_ascii=False))
