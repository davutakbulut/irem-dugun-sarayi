...
def verify_isolated_previews():
    ...
    # Apply Quiet Luxury theme globally to check if preview cards remain isolated
    ...
    res = client.eval_js("""
        const classicCard = Array.from(document.querySelectorAll('div')).find(d => d.innerText && d.innerText.includes('Klasik Şampanya Altını'));
        const classicBtn = classicCard ? Array.from(classicCard.querySelectorAll('div')).find(el => el.innerText === 'Canlı Buton') : null;
        const classicBtnRadius = classicBtn ? classicBtn.style.borderRadius : null;
        const classicBtnBg = classicBtn ? classicBtn.style.background : null;

        return {
            classicCardFound: !!classicCard,
            classicSampleBtnFound: !!classicBtn,
            classicSampleBtnBorderRadius: classicBtnRadius,
            classicSampleBtnBackground: classicBtnBg,
            is16pxRoundedCurve: classicBtnRadius === '16px',
            isGoldGradient: classicBtnBg.includes('gradient'),
            isolatedPreviewsWorking: classicBtnRadius === '16px' && classicBtnBg.includes('gradient')
        };
    """)
