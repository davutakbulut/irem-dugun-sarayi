...
def test_nordic_light_fixes():
    ...
    res = client.eval_js("""
        const pageHeaderBlock = document.querySelector('.glass-panel .flex-col');
        const badgeSpan = Array.from(document.querySelectorAll('span')).find(s => s.innerText.includes('Rezervasyon Oluşturma & Kiralama'));
        const headingH2 = Array.from(document.querySelectorAll('h2')).find(h => h.innerText.includes('Hayalinizdeki Düğünü Birlikte Planlayalım!'));
        const selectedBadge = Array.from(document.querySelectorAll('.gold-button')).find(b => b.innerText.includes('SEÇİLDİ'));
        const selectedBadgeBg = selectedBadge ? getComputedStyle(selectedBadge).backgroundColor : null;
        const svgIcons = document.querySelectorAll('svg');

        return {
            pageHeaderFlexColLayout: !!pageHeaderBlock,
            badgeSpanFound: !!badgeSpan,
            headingH2Found: !!headingH2,
            noTextOverlap: pageHeaderBlock && badgeSpan && headingH2,
            selectedBadgeFound: !!selectedBadge,
            selectedBadgeNordicBgColor: selectedBadgeBg,
            isNordicNavy: selectedBadgeBg === 'rgb(15, 23, 42)', // #0F172A
            svgIconsCount: svgIcons.length,
            corporateSvgIconsActive: svgIcons.length > 5
        };
    """)
