...
def test_profile_navigation_deep():
    ws_url = get_target()
    client = CDPClient(ws_url)

    print("--- Testing Direct Page Reload with Hash http://localhost:8000/#/profil ---")
    client.call("Page.navigate", {"url": "http://localhost:8000/#/profil"})
    time.sleep(1.5)

    title = client.eval_js("return document.title;")
    hash_val = client.eval_js("return window.location.hash;")
    print(f"Title: {title}, Hash: {hash_val}")

    page_text = client.eval_js("return document.body.innerText;")
    print("\nPage InnerText Sample:")
    print(page_text[:600] if page_text else "EMPTY")

    profile_audit = client.eval_js("""
        const profileHeader = Array.from(document.querySelectorAll('h2, h3')).find(h => h.innerText.includes('İrem Yılmaz') || h.includes('Profil'));
    ...
    print(json.dumps(profile_audit, indent=2, ensure_ascii=False))

if __name__ == '__main__':
    test_profile_navigation_deep()
