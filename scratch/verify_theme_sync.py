...
def verify_sync():
    ...
    # Select Quiet Luxury card & click Save button
    res1 = client.eval_js("""
        const quietCard = Array.from(document.querySelectorAll('div')).find(d => d.innerText && d.innerText.includes('Quiet Luxury & Minimalist Architecture'));
        ...
        const selectedBadge = Array.from(document.querySelectorAll('span')).find(s => s.innerText.includes('SEÇİLDİ'));

        return {
            selectedBadgeText: selectedBadge ? selectedBadge.innerText.trim() : null,
            badgeActive: !!selectedBadge
        };
    """)
    print("Step 1 (Card Selection & Save):", json.dumps(res1, indent=2, ensure_ascii=False))

    # Perform F5 Reload
    client.call("Page.navigate", {"url": "http://localhost:8000/#/ayarlar/gorunum"})
    time.sleep(1.2)

    # Step 2: Check 0ms Instant Load after F5 reload
    res2 = client.eval_js("""
        const attr = document.documentElement.getAttribute('data-ui-theme');
        const selectedBadge = Array.from(document.querySelectorAll('span')).find(s => s.innerText.includes('SEÇİLDİ'));
        const storedTheme = localStorage.getItem('irem_cache_theme_color');

        return {
            uiThemeAttributeOnF5Reload: attr,
            storedThemeInLocalStorage: storedTheme,
            selectedBadgeFoundOnReload: !!selectedBadge,
            instant0msPaintSuccess: attr === 'elite-luxury'
        };
    """)
    print("Step 2 (F5 Reload & Instant Restore):", json.dumps(res2, indent=2, ensure_ascii=False))
