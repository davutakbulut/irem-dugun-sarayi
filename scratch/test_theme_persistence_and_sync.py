...
def test_theme_sync_and_persistence():
    ws_url = get_target()
    client = CDPClient(ws_url)

    # 1. Set localStorage theme to 'nordic-light'
    print("Setting localStorage theme to 'nordic-light'...")
    client.eval_js("localStorage.setItem('irem_cache_theme_color', JSON.stringify('nordic-light'));")

    # Reload page (F5)
    client.call("Page.navigate", {"url": "http://localhost:8000/#/ayarlar/gorunum"})
    time.sleep(1.2)

    # Verify 0ms instant paint attribute & SEÇİLDİ badge
    res1 = client.eval_js("""
        const attr = document.documentElement.getAttribute('data-ui-theme');
        const selectedBadges = Array.from(document.querySelectorAll('span')).filter(s => s.innerText.includes('SEÇİLDİ')).map(s => s.innerText.trim());

        return {
            theme: 'nordic-light',
            uiThemeAttributeOnReload: attr,
            selectedBadgesCount: selectedBadges.length,
            instant0msPaintSuccess: attr === 'nordic-light'
        };
    """)
    print("Nordic Light Persistence Test:", json.dumps(res1, indent=2, ensure_ascii=False))

    # 2. Set localStorage theme to 'elite-luxury'
    print("\nSetting localStorage theme to 'elite-luxury'...")
    client.eval_js("localStorage.setItem('irem_cache_theme_color', JSON.stringify('elite-luxury'));")
    ...
    """)
    print("Elite Luxury Persistence Test:", json.dumps(res2, indent=2, ensure_ascii=False))

if __name__ == '__main__':
    test_theme_sync_and_persistence()
