...
def test_modal_click():
    ...
    # Click day cell date number span directly (e.g. <span>15</span>)
    res = client.eval_js("""
        const spans = Array.from(document.querySelectorAll('span')).filter(s => s.innerText === '15');
        const daySpan = spans.find(s => s.parentElement && s.parentElement.parentElement && s.parentElement.parentElement.className && s.parentElement.parentElement.className.includes('min-h'));
        if (daySpan) {
            daySpan.parentElement.parentElement.click();
        }
    ...
    """)
    print("Modal Click Result:", json.dumps(res, indent=2, ensure_ascii=False))

    # Click 'İçin Yeni Rezervasyon Oluştur' button inside DayDetailModal
    res_btn = client.eval_js("""
        const newResBtn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('İçin Yeni Rezervasyon Oluştur'));
    ...
    """)
    print("Auto-Fill Redirection Result:", json.dumps(res_btn, indent=2, ensure_ascii=False))
