...
def test_new_reservation_page_load():
    ws_url = get_target()
    client = CDPClient(ws_url)

    client.call("Page.navigate", {"url": "http://localhost:8000/#/rezervasyon-olustur"})
    time.sleep(1.2)

    res = client.eval_js("""
        const pageTitle = document.querySelector('h2');
        const titleText = pageTitle ? pageTitle.innerText : '';
        const bodyText = document.body ? document.body.innerText : '';
        const hasUnitPriceError = bodyText.includes('unitPrice is not defined') || bodyText.includes('ReferenceError');

        return {
            pageLoadedSuccessfully: titleText.includes('Yeni Rezervasyon') || titleText.includes('Rezervasyon'),
            titleText: titleText,
            hasUnitPriceError: hasUnitPriceError
        };
    """)
    print("New Reservation Page Load Result:", json.dumps(res, indent=2, ensure_ascii=False))

if __name__ == '__main__':
    test_new_reservation_page_load()
