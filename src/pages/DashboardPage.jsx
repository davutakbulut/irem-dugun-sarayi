...
import { generateSmartAIRecommendations } from '../data/initialData';

export function DashboardPage({ activeRole, venues = [], reservations = [], financialStats, onNewResClick, onTabChange, onConvertToCampaign, onUpdateVenuePrice }) {
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-brand-border/40 shadow-sm">
              <div className="text-xs text-slate-500 dark:text-gray-400">Toplam Düğün Salonu</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-brand-border space-y-4">
              <div className="flex justify-between items-center border-b pb-3 border-slate-200 dark:border-brand-border">
                ...
                <span className="text-[10px] font-bold bg-amber-500/10 text-amber-700 dark:text-gold-400 px-2.5 py-1 rounded-full">Kiralama Sayısı</span>
              </div>
              <div className="h-44 flex items-end justify-between gap-3 pt-6 px-2">
                {venuePreferenceData.map((item, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center h-full justify-end group relative">
                    <div className="text-[10px] font-bold text-amber-700 dark:text-gold-400 mb-1">{item.count} Etkinlik</div>
                    <div
                    ...
                    />
                    <span className="text-[10px] font-bold text-slate-600 dark:text-gray-400 mt-2 truncate w-full text-center">{item.name.replace('Salon', '').trim()}</span>
                  </div>
                ))}
              </div>
            </div>
