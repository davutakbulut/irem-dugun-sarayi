...
const ROLE_NAMES = {
  ...
  sosyal_medyaci: 'Sosyal Medya Müdürü',
  musteri: 'Müşteri Görünümü'
};

export function ProfilePage({ currentUser, activeRole = 'SuperAdmin', onSaveProfile = () => {}, showToast = () => {}, onRoleChange = () => {} }) {
  const [name, setName] = useState(currentUser?.name || 'Davut Akbulut');
  const [email, setEmail] = useState(currentUser?.email || 'admin@iremdugunsarayi.com');
  const [phone, setPhone] = useState(currentUser?.phone || '+90 532 000 0000');
  const [avatar, setAvatar] = useState(currentUser?.avatar || 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=200&q=80');
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in pb-12">
      ...
      <div className="glass-panel p-6 rounded-3xl border border-amber-500/30 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm">
        <div className="flex items-center space-x-4">
          <img src={avatar} alt={name} className="w-16 h-16 rounded-full border-2 border-amber-500/60 shadow object-cover" />
          <div>
          ...
          <div>
            ...
            {isAdmin ? (
              ...
              <div className="p-2.5 bg-slate-100 dark:bg-brand-dark rounded-xl border border-slate-200 dark:border-brand-border text-slate-500 dark:text-gray-400 font-bold">
                {ROLE_NAMES[activeRole] || activeRole} — Roller sadece Sistem Yöneticisi tarafından değiştirilebilir.
              </div>
            )}
          </div>
        </div>
