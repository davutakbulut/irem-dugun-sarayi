      ...
      {editingRes && editForm && (
        <div className="fixed inset-0 z-[99999] bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-fade-in">
          <div className="bg-white dark:bg-brand-card border border-amber-500/50 rounded-3xl max-w-4xl w-full p-5 sm:p-6 space-y-5 shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto custom-scrollbar my-auto">
            ...
            {/* EDIT MODAL FOOTER */}
            <div className="pt-3 border-t border-slate-200 dark:border-brand-border flex justify-between items-center text-xs font-bold">
              {editError && <span className="text-red-500 font-bold animate-pulse">⚠️ Müşteri adı ve birincil telefon zorunludur!</span>}
              <div className="flex space-x-3 ml-auto">
                <button onClick={() => setEditingRes(null)} className="px-4 py-2 bg-slate-100 dark:bg-brand-dark text-slate-600 dark:text-gray-300 rounded-xl font-bold">İptal</button>
                <button
                  onClick={() => {
                    ...
                    const disc = Number(editForm.discountAmount || 0);
                    const tot = Math.max(0, venueCost + srvCost - disc);
                    const dep = Number(editForm.depositPaid || 0);
                    const rem = Math.max(0, tot - dep);
