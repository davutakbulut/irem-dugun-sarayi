          ...
          <div id="customer-section" className={`glass-panel p-6 rounded-3xl space-y-4 shadow-sm border transition ${customerError ? 'border-2 border-red-500 bg-red-500/5' : 'border-slate-200 dark:border-brand-border'}`}>
            <div className="border-b border-slate-200 dark:border-brand-border pb-3 space-y-3">
              ...
              <div className="flex flex-wrap justify-center sm:justify-start gap-2 text-xs font-bold">
                <button onClick={() => { setCustomerMode('new'); setCustomerError(false); }} className={`px-3.5 py-2 rounded-xl border transition ${customerMode === 'new' ? 'gold-button shadow' : 'bg-slate-100 dark:bg-brand-dark text-slate-600 dark:text-gray-400'}`}>
                  ➕ Otomatik Yeni Üyelik Oluştur
                </button>
                <button onClick={() => { setCustomerMode('existing'); setCustomerError(false); }} className={`px-3.5 py-2 rounded-xl border transition ${customerMode === 'existing' ? 'gold-button shadow' : 'bg-slate-100 dark:bg-brand-dark text-slate-600 dark:text-gray-400'}`}>
